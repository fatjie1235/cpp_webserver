...
bool HttpRequest::processHttpRequest(HttpResponse *response)
{
    ...
    if (ret == -1)
    {
        ...
        return true; // ← 处理成功（虽然文件不存在，但响应已准备好）
    }
...
void HttpRequest::sendDir(string dirName, Buffer *sendBuf, int cfd)
{
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        // 用 snprintf + 剩余空间计算，防止溢出
        int used = strlen(buf);
        int remaining = sizeof(buf) - used - 1;
        if (remaining <= 0)
        {
            // buf 满了，先发送出去再继续
...
void HttpRequest::sendFile(string fileName, Buffer *sendBuf, int cfd)
{
    ...
    if (fd < 0)
    {
        perror("open file");
        // 返回一个简单的错误页面
        const char *errMsg = "<html><body><h1>500 Internal Server Error</h1></body></html>";
        char header[256];
        sprintf(header, "HTTP/1.1 500 Internal Server Error\r\n"
                        "Content-type: text/html; charset=utf-8\r\n"
                        "Content-length: %zu\r\n\r\n",
                strlen(errMsg));
        sendBuf->appendString(header);
        sendBuf->appendString(errMsg);
        sendBuf->sendData(cfd);
        return;
    }
#if 1
