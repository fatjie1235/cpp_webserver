...
int TcpConnection::destroy(void* arg)
{
    ...
    if (conn != nullptr)
    {
        // 注意：delete conn 会触发 ~TcpConnection
        // ~TcpConnection 内部会调 freeChannel
        // freeChannel 会从 channelMap 移除并 delete channel
        // 所以 destroy 回调只需要 delete conn 即可
        delete conn;
    }
    return 0;
}
...
TcpConnection::~TcpConnection()
{
    ...
    delete m_request;
    delete m_response;
    // freeChannel 会 close(fd) 并 delete channel
    // 不要在这里再 delete this，所以 destroy 回调中需要判空
